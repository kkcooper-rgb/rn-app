...
import Badg from '@ant-design/react-native/lib/badge';
...
class GoodDetail extends Component {
  ...
  componentDidMount() {
    ...
    this.handleTabBarBadge(this.props.cartData.detailData)
  }
  getGoodsDetailData() {
    const {route} = this.props;
    console.log(this.props, api.getGoodsDetail, 'this.props');
    request(`${api.getGoodsDetail}?goods_id=${route.params.goods_id}`)
      .then(res => {
  ...
  render() {
    ...
    const {goods_info, goods_button_info, BadgData} = this.state;
      ...
      <View style={{display: 'flex', flex: 1, position: 'relative'}}>
        ...
        <View
          ...
          {goods_button_info.map((val, ii) => {
            return (
              // <Badg text={val.title === '购物车' ?1:0} key={ii} >
              <View key={ii} style={val.buttonStyle}>
                <TouchableWithoutFeedback
                  onPress={() => {
                    console.log('chufa');
                    if (val.title === '加入购物车') {
                      ...
                      const goods = {
                        ...
                        goods_count: 1, // 商品的数量
                        goods_small_logo:
                          this.state.goods_info.goods_small_logo, // 商品的图片
                        goods_state: true, // 商品的勾选状态
                      };
                      let newcartData = cartData.detailData;
                      let goodsFilterData =
                        (newcartData.length &&
                          newcartData.filter(fitem => {
                            if (
                              fitem.goods_id === this.state.goods_info.goods_id
                            ) {
                      ...
                      this.handleTabBarBadge(newcartData)
                  ...
                  <View
                    ...
                    {val.icon && (
                      <Badg text={val.title === '购物车' ? BadgData : 0}>
                        <View style={{width:40,display:'flex',justifyContent:'center',flexDirection:'row'}}>
                          <Ionicons
                            name={val.icon}
                            size={24}
